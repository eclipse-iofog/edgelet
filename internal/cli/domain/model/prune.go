...
// PruneResult carries model prune outcome.
type PruneResult struct {
	Human string
	Data  map[string]any
}

const modelPruneUsage = "usage: edgelet model prune [dangling]"

// Prune removes unreferenced model artifacts.
func Prune(client run.EdgeletAPIClient, args []string) (*PruneResult, error) {
	...
	mode, err := prune.ParseModelMode(args, modelPruneUsage)
	if err != nil {
		return nil, err
	}
	path := "/v1/models:prune"
	if mode != "" {
		path += "?mode=" + mode
	}
