...
// PullRequest carries model pull options.
...
// PullResult is the model pull command outcome.
type PullResult struct {
	Data  map[string]any
	Human string
}

// Pull starts an async model pull and waits until it finishes.
func Pull(ctx context.Context, api run.EdgeletAPIClient, uiProgress *ui.UI, req PullRequest) (*PullResult, error) {
	...
	if req.hasSpec() && (strings.TrimSpace(req.Repo) == "" || req.RegistryID <= 0) {
		return nil, run.NewCLIError(run.CodeInvalidArgument, "repo and --registry are required when specifying a model", nil)
	}
	...
	startResult, err := api.Request("POST", "/v1/models:pull", body)
	if err != nil {
		return nil, run.MapAPIError(err)
	}
	operationID := client.OperationIDFromStart(startResult)
	if operationID == "" || operationID == "<unknown>" {
		return nil, run.NewCLIError(run.CodeInternal, "missing model pull operationId in response", nil)
	}

	progress := client.PollProgress{
		UI:           uiProgress,
		PercentLabel: "pulling model",
	}
	if uiProgress != nil {
		spin := uiProgress.StartSpinner("Pulling model...")
		defer spin.Stop()
	...
	final, _, err := client.PollAsyncOperation(ctx, client.PollConfig{
		...
		Timeout:  client.PollTimeoutFor("model-pull"),
	}, func() (map[string]any, error) {
		return api.Request("GET", "/v1/models:pull/"+operationID, nil)
	}, progress)
	if err != nil {
		return nil, run.MapAPIError(err)
	}
