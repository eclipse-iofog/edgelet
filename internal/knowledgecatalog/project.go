...
const (
	...
	catalogDirName    = "knowledge"
	dataSymlink       = "..data"
	catalogMetaFile   = "..catalog"
	bindMountDirMode  = 0755 // #nosec G301 -- bind-mount parent must be traversable for non-root containers
	bindMountFileMode = 0644 // #nosec G306 -- projected knowledge files are readable inside the container
)
...
// ProjectItem is one Ready Knowledge to materialize under {name}/.
...
// HostDir is {diskDirectory}/volumes/microservices/{msUUID}/knowledge.
...
func Project(diskDirectory, msUUID string, catalog *models.KnowledgeCatalog, items []ProjectItem) (*ProjectResult, error) {
	...
	mountChanged := models.KnowledgeCatalogMountNeedsRecreate(lastCatalog(hostDir), cloned)
...
func snapshotToCatalog(s *snapshot) *models.KnowledgeCatalog {
	if s == nil {
		return nil
	}
	c := &models.KnowledgeCatalog{
		BindPath:    s.BindPath,
		Permissions: s.Permissions,
		Items:       make([]models.KnowledgeCatalogItem, 0, len(s.Items)),
	}
	for _, item := range s.Items {
		c.Items = append(c.Items, models.KnowledgeCatalogItem{Name: item.Name})
	}
	return c
}
...
func syncNameLinks(hostDir string, items []models.KnowledgeCatalogItem) error {
...
func linkOrCopyFile(src, dst string) error {
	...
	in, err := os.Open(src) // #nosec G304 -- src is under the knowledge content tree
	if err != nil {
		return err
	}
...
func lastCatalog(hostDir string) *models.KnowledgeCatalog {
...
func mountChangedFromDisk(hostDir string, desired *models.KnowledgeCatalog) bool {
	return models.KnowledgeCatalogMountNeedsRecreate(lastCatalog(hostDir), desired)
}
