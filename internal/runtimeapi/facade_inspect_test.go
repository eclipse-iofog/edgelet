...
func TestFacadeGetRuntimeMicroservice_KnowledgeCatalogAndWaitStatus(t *testing.T) {
	f := NewFacade()
	if err := f.db.Open(t.TempDir()); err != nil {
		t.Fatalf("open store: %v", err)
	}
	...
	if err := f.db.UpsertLocalKnowledge(&models.LocalKnowledge{
		Name:       "product-docs",
		Source:     models.KnowledgeSourceLocal,
		Repo:       "org/product-docs",
		RegistryID: 1,
		State:      models.KnowledgeStatePulling,
	}); err != nil {
...
`) + "\n"
	if err := f.db.UpsertLocalWorkload(&models.LocalDeployedMicroservice{
		LocalUUID:        "local-inspect-knowledge",
		ApplicationName:  "edgelet",
		MicroserviceName: "infer",
		SourceName:       "local-cli",
		ManifestYAML:     manifest,
	...
	item, err := f.GetRuntimeMicroservice("local-inspect-knowledge")
	if err != nil {
		t.Fatalf("inspect: %v", err)
	}
	catalog, ok := item["knowledge"].(map[string]any)
	if !ok {
		t.Fatalf("expected knowledge catalog on inspect, got %#v", item)
	}
	if catalog["bindPath"] != "/knowledge" || catalog["permissions"] != "ro" {
		t.Fatalf("unexpected knowledge catalog fields: %#v", catalog)
	}
	items, ok := catalog["items"].([]map[string]any)
	if !ok || len(items) != 1 || items[0]["name"] != "product-docs" {
		t.Fatalf("expected knowledge catalog item names, got %#v", catalog["items"])
	}
	statusText, ok := item["statusText"].(string)
	if !ok {
		t.Fatalf("expected statusText string, got %#v", item["statusText"])
	}
	if !strings.Contains(statusText, "product-docs") || !strings.Contains(statusText, models.KnowledgeStatePulling) {
		t.Fatalf("expected wait status with knowledge name and state, got %q", statusText)
	}
}
