...
func identityForDesired(reg *models.Registry, row *models.LocalKnowledge) string {
	if row == nil {
		return ""
	}
...
func specFingerprint(row *models.LocalKnowledge) string {
	if row == nil {
		return ""
	}
	return fmt.Sprintf("%d|%s|%s|%s|%s",
...
func identityMatchesManifest(desiredKey string, onDisk modelpull.OnDiskManifest, reg *models.Registry, row *models.LocalKnowledge) bool {
